# -*- coding: utf-8 -*-
"""
Created on Wed Jun 14 14:55:14 2023
...
##User input##
output_parent_directory = r"C:\Users\lawashburn\Documents\DBpep_v2\Validation_w_Kellen_Motif_Nhu_Raw_Files\PEAKS_oursoftware_compare_brain_only\DB_search_optimize\03_precursor_AMM_optimize_v2" #folder in which all output directories will be generated
...
default_peptide_representation = 'none'
...
def db_seq_mass_compile(fasta_to_df):
    ...
    for b in fasta_to_df:
        ...
        for sequence in final_seq_list:
            if '(Gln->pyro-Glu)' or '(Glu->pyro-Glu)' in sequence:
...
def precursor_amm(db,exp_precursor):
    ...
    for a in range(0,(len(db_sorted)-1)):
        ...
        # file_path = peptide_report_output + '\\db_filtered.csv'
        # with open(file_path,'w',newline='') as filec:
        #         writerc = csv.writer(filec)
        #         db_filtered.to_csv(filec,index=False)  
                
        # file_path = peptide_report_output + '\\exp_precursor_sorted.csv'
        # with open(file_path,'w',newline='') as filec:
        #         writerc = csv.writer(filec)
        #         exp_precursor_sorted.to_csv(filec,index=False)  


        merge_match2 = pd.merge_asof(exp_precursor_segment,db_filtered, left_on='Precursor actual monoisotopic mass', 
                                    right_on='Precursor theoretical monoisotopic mass',
                                    tolerance = precursor_temp_cutoff, allow_exact_matches=True,direction='forward') 

        merge_match3 = pd.merge_asof(exp_precursor_segment,db_filtered, left_on='Precursor actual monoisotopic mass', 
                                    right_on='Precursor theoretical monoisotopic mass',
...
## Definition storage end
fasta_w_mass = pd.DataFrame()

if use_predefined_db == True:
    fasta_w_mass_prelim = pd.read_csv(db_path)
    fasta_w_mass_prelim['Precursor theoretical monoisotopic mass'].round(decimals = 4)
    fasta_w_mass = pd.concat([fasta_w_mass,fasta_w_mass_prelim])
elif use_predefined_db == False:
    print('yes')
    fasta_to_df_ordered = []
    ...
    fasta_w_mass_prelim = db_seq_mass_compile(fasta_to_df)
    print(fasta_w_mass_prelim)
    fasta_w_mass = pd.concat([fasta_w_mass,fasta_w_mass_prelim])
...
for file_category in raw_conv_mzml_storage:
    ...
    if default_peptide_representation == 'none':
        fasta_w_mass = fasta_w_mass[fasta_w_mass["Sequence"].str.contains("\(") == False]
        print(fasta_w_mass)
        initial_psms = perform_db_search(fasta_w_mass,exp_precursor,peptide_report_output)
        
        # file_path = peptide_report_output + '\\initial_psms_report.csv'
        # with open(file_path,'w',newline='') as filec:
        #         writerc = csv.writer(filec)
        #         finalized_psms.to_csv(filec,index=False)
    ...
    # for a in range(0,subsequent_matching_rounds):

...
db_search_inital_output = r"C:\Users\lawashburn\Documents\DBpep_v2\Validation_w_Kellen_Motif_Nhu_Raw_Files\NV_output_DB_search_results"
...
output_directory = r"C:\Users\lawashburn\Documents\DBpep_v2\Validation_w_Kellen_Motif_Nhu_Raw_Files\PEAKS_oursoftware_compare"
...
for sample in samples:
    ...
    # precursor_db_search_results_path = db_search_inital_output + '\\2021_0817_' + sample + '\\precursor_amm_results.csv'
    # precursor_db_search_results = pd.read_csv(precursor_db_search_results_path)
    # precursor_db_search_results['Sequence (no mods)'] = precursor_db_search_results['Sequence'].str.replace(r"\([^)]*\)","")
    # initial_precursor_db_search_results_filtered = precursor_db_search_results.drop_duplicates(subset='Sequence (no mods)')
    # precursor_unqiue_peptide_results = initial_precursor_db_search_results_filtered['Sequence (no mods)'].values.tolist() #Results not FDR filtered

    initial_db_search_results_path = db_search_inital_output + '\\2021_0817_' + sample + '\\results_with_correlation_w_motif_full.csv'
    ...
    # results_precursor_accession = [] #Accession of precursor resuts
    # for f in precursor_unqiue_peptide_results:
    #     if f in db_seqs:
    #         np_database_filtered = np_database[np_database['Sequence'] == f]
    #         np_database_filtered = np_database_filtered.reset_index(drop=True)
    #         accession_to_report = np_database_filtered['Accession'][0]
    #         results_precursor_accession.append(int(accession_to_report))
    #     else:
    #         pass
    ...
    # for z in peaks_unique_peptides:
    #     if z in results_precursor_accession:
    #         pass
    #     else:
    #         unique_to_peaks_precursor.append(z)
...
##
# unique_to_peaks_precursor_no_dups = []
# for i in unique_to_peaks_precursor:
#     if i not in unique_to_peaks_precursor_no_dups:
#         unique_to_peaks_precursor_no_dups.append(i)
#     else:
#         pass

# precursor_results_storage_accession = []
# precursor_results_storage_peptide = []
# precursor_results_storage_instances = []

# for p in unique_to_peaks_precursor_no_dups:
#     instances = unique_to_peaks_precursor.count(p)
#     np_database_filtered = np_database[np_database['Accession'] == p]
#     np_database_filtered = np_database_filtered.reset_index(drop=True)
#     sequence_to_report = np_database_filtered['Sequence'][0]
#     precursor_results_storage_peptide.append((sequence_to_report))
#     precursor_results_storage_accession.append(p)
#     precursor_results_storage_instances.append(instances)
...
##
# precursor_unique_to_PEAKS = pd.DataFrame()
# precursor_unique_to_PEAKS['Sequence'] = precursor_results_storage_peptide
# precursor_unique_to_PEAKS['Accession'] = precursor_results_storage_accession
# precursor_unique_to_PEAKS['Instances'] = precursor_results_storage_instances


# file_path = output_directory + '\\unique_to_PEAKS_from_precursor_db_search_results.csv'
# with open(file_path,'w',newline='') as filec:
#         writerc = csv.writer(filec)
#         precursor_unique_to_PEAKS.to_csv(filec,index=False)
...
for file_category in raw_conv_mzml_storage:
  ...
  #%%  
    ...
    db_sorted = fasta_w_mass.sort_values(by = 'Precursor theoretical monoisotopic mass') #sort database monoisotopic mass and experimental for mergeasof
    db_sorted['Precursor theoretical monoisotopic mass'].round(decimals = 3)
    db_sorted = db_sorted[db_sorted['Precursor theoretical monoisotopic mass'] > 966.526]
    db_sorted = db_sorted[db_sorted['Precursor theoretical monoisotopic mass'] < 966.528]
    ...
    merge_match_count = merge_match_merge_again.drop_duplicates(subset=['Scan','Sequence_y'])

...
for scan in scan_candidate_list:
    ...
    if len(score_scan_filtered_psm_candidate) == 1:
        ...
        if count_min_instance > 0:
            score_score_scan_filtered_psm_candidate = score_scan_filtered_psm_candidate[score_scan_filtered_psm_candidate['Correlation value'] > 0]
            if len(score_score_scan_filtered_psm_candidate) > 0:
                psm_entry = score_score_scan_filtered_psm_candidate.head(1)
                final_psm = pd.concat([final_psm,psm_entry])
            else:
                score_scan_filtered_psm_candidate = score_scan_filtered_psm_candidate.sort_values(by='Sequence coverage',ascending=False)

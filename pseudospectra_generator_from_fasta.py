# -*- coding: utf-8 -*-
"""
Created on Tue Jan 24 09:49:34 2023
...
##User input##
output_parent_directory = r"C:\Users\lawashburn\Documents\DBpep_v2\XCorr_Opt\XCorr_validation\20230131" #folder in which all output directories will be generated
...
for seq in pg_filtered_list:
    ...
    for sequence in finalized_mod_list:
        ...
        theoretical_peaks = theoretical_spectra_generator(sequence)
    
    for jj in precursor_charges:
        precursor_mz = (mono_mass + (h_mass * jj))/jj
        for ll in fragment_charges:
            theoretical_peaks['m/z'] = ((theoretical_peaks['Fragment theoretical Monoisotopic Mass'] ) + (h_mass * ll))/ll
            theoretical_peaks['resolution'] = 50000
            theoretical_peaks['charge'] = ll
            
            intensity_storage = []
            for aa in range(0,len(theoretical_peaks)):
                intensity_storage.append(randint(1550,1600))
            
            theoretical_peaks['intensity'] = intensity_storage
            theoretical_peaks['MS2'] = precursor_mz
            
            scan = (scan_count[-1])+1
            scan_count.append(scan)
            
            theoretical_peaks['scan_number'] = scan
            theoretical_peaks['precursor_charge'] = jj
            theoretical_peaks['sequence'] = sequence
            
            theoretical_peaks = theoretical_peaks.round({'Fragment theoretical Monoisotopic Mass': 4,
                                                          'm/z': 4,
                                                          'MS2': 4})
            compilation = pd.concat([compilation,theoretical_peaks])
            
writePath = output_parent_directory + '\\pseudospectra_nomods_1DF.txt'

compilation.to_csv(writePath, header=['ion','Fragment theoretical Monoisotopic Mass','m/z','resolution','charge','intensity','MS2','scan_number','precursor_charge','sequence'], index=None, sep=',', mode='a')
